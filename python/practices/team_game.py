...
def hangman(): #(Danny):this is a function so the game loop can access it every time by just reading 'hangman()'
...
def game_loop(): #(Danny):this is a function because after the user wins or loses, it can access this whole loop again depending if they want to play again or not
    ...
    while True: #(Danny):while true loops last forever, so the loop runs every single time and asks the same question to the user
        ...
        if display_words == word: #(Danny):since display_words adds letters over time, it would check if the display_words is the same as the word the user needs to guess, if all the blank spots are filled it would perform this action, if not, it would go to else
        ...
        elif attempts == 4: #(Danny):since the display_words are not the same as the word, it checks if the attempts are 4, because that's the limit, if it is, it performs the following:
        ...
        list_of_guessed_letters.append(guess) #(Danny):it adds the guessed letter to the blank list, therefore giving the code the list to check if the letter is in the word, giving it access to the letter the user guessed
        if guess not in word: #(Danny):it's a conditional that checks if the guessed letter was not in the word, if it's true, it performs the following:
            attempts += 1 #(Danny):it adds 1 to attempts since the user guessed wrong, and it gives the hangman loop an instruction to perform depending on the attempts (what hangman to print) and whether the user loses depending on the the number of attempts


game_loop() #(Danny):since the above code is all the definition for game_loop(), it finally actually performs this, it's a variable so the code runner can access it again in case the user wants to play again
...
if play_again == "yes": #(Danny):if the user chooses to play again (which the code checks whether is it yes or no), the loop will restart
    ...
    attempts = 0 #(Danny):this resets the attempts to 0 because if it didn't, the user would immediately lose
